...
export async function closeSupport(message:Message) {
    ...
    const embed = generateEmbedSupportMessage('Ticket clos', `Clôture du ticket par ${message.author.toString()}`);
    channel.send({embed});
}
...
export async function deleteSupport(channel:TextChannel) {
    const db = Database.getInstance();
    await db.execQueryWithParams('UPDATE channel SET actif = 0 WHERE channelUniqueId = ?', [channel.id]);
    const log_channel = channel.guild.channels.cache.find(c => c.name === "logs_bot") as TextChannel;
